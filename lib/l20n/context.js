...
define(function (require, exports) {
  ...
  var LangPackClient = require('./platform/langpack').LangPackClient;

  // register globals with RetranslationManager
  require('./platform/globals');

  function Resource(id, parser, lpc) {
    var self = this;
    ...
    function build(nesting, callback, sync) {
      ...
      if (self.source) {
        ...
        lpc.getResource(self, parse, sync);
      }
      ...
      function buildImports() {
        ...
        imports.forEach(function(imp) {
          ...
          var res = new Resource(uri, parser, lpc);
  ...
  function Context(id) {
    ...
    var _lpc = new LangPackClient(this);
    ...
    function add(text, locale) {
      var res = new Resource(null, _parser, _lpc);
      res.source = text;
      res.locale = locale;
    ...
    function link(uri, locale) {
      ...
      if (!locale.hasResource(uri)) {
        var res = new Resource(uri, _parser, _lpc);
        res.locale = locale;
        locale.resources.push(res);
      }
    }
    ...
    function registerLocales(defLocale, available) {
      ...
      var registered = [];
      // uniquify `available` into `registered`
      available.forEach(function(loc) {
        if (typeof loc !== 'string') {
          throw new ContextError('Language codes must be strings');
        }
        if (registered.indexOf(loc) === -1) {
          registered.push(loc);
        }
      });

      _lpc.registerLocales(defLocale, registered);
    }
    ...
    function registerLocaleNegotiator(negotiator) {
      ...
      _lpc.registerLocaleNegotiator(negotiator);
    }
    ...
    function requestLocales() {
      ...
      _lpc.negotiateLocales(_requested, freeze);
    }
